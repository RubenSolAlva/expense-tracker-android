...
class SaveTransactionUseCaseTest {
    ...
    companion object {
        ...
        internal const val FINANCIAL_ACCOUNT_ID_MOCKED = 0
    ...
    private val accountMocked = mockk<Account> {
        every { id } returns FINANCIAL_ACCOUNT_ID_MOCKED
    }
    ...
    fun `On invoke SaveTransactionUseCase should call repository to save transaction`() {
        runBlocking {
            saveTransactionUseCase(
                accountMocked,
                transactionCategoryMocked,
                AMOUNT_MOCKED
            )
        }

        coVerify { transactionsRepository.saveTransaction(any(), any(), any(), any()) }
    }
}
